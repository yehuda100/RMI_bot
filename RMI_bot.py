...
def get_color(update, context):
    ...
    global DATA
    DATA[update.effective_user.id]['color'] = update.message.text
...
def get_text(update, context):
    ...
    global DATA
    DATA[update.effective_user.id]['text'] = update.message.text
...
def get_quoted(update, context):
    global DATA
    DATA[update.effective_user.id]['quoted'] = update.message.text

    context.bot.send_chat_action(chat_id=update.effective_user.id, action=ChatAction.UPLOAD_PHOTO)

    img = imgs.bot((update.effective_user.id,) + tuple(DATA[update.effective_user.id].values()))
